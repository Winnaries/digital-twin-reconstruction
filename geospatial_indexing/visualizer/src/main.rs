...
use glium::glutin::event::{ElementState, Event, MouseButton, VirtualKeyCode, WindowEvent};
...
fn main() {
    ...
    let program_for_plane =
        glium::Program::from_source(&display, plane::VERTEX_SHADER, plane::FRAGMENT_SHADER, None).unwrap();

    let speed = 0.1f32;
    let mut current_time = std::time::Instant::now();
    let mut camera: Camera = Default::default();

    let mut planes: Vec<plane::Plane> = Vec::new();

    for fl in [-12.0460, -8.9141, -4.4212, 0.1752, 3.2043] {
        let mut plane = plane::Plane::default();
        plane.width = 160.0;
        plane.length = 160.0;
        plane.set_position(40.0, fl, -32.0);
        planes.push(plane);
    }

    let plane_buffers: Vec<_> = planes.iter().map(|x| VertexBuffer::new(&display, &x.vertices).unwrap()).collect(); 
    ...
    let world = glm::diagonal4x4(&glm::vec4(1.0f32, 1.0f32, 1.0f32, 1.0f32));
    let world = glm::scale(&world, &glm::vec3(1.0, 3.0, 1.0)); 
    ...
    event_loop.run(move |ev, _, control_flow| {
        ...
        *control_flow = ControlFlow::WaitUntil(next_frame_time);
        ...
        if movement.forward {
            camera.travel(0.0, -distance)
        }
        if movement.backward {
            camera.travel(0.0, distance)
        }
        if movement.left {
            camera.travel(-distance, 0.0)
        }
        if movement.right {
            camera.travel(distance, 0.0)
        }
        ...
        planes.iter().zip(&plane_buffers).for_each(|(plane, buf)| {
            target
                .draw(
                    buf,
                    plane.indices(),
            ...
            Event::WindowEvent { event, .. } => match event {
                ...
                WindowEvent::KeyboardInput { input, .. } => {
                    if let Some(key) = input.virtual_keycode {
                        match key {
                            VirtualKeyCode::A => {
                                movement.left = input.state == ElementState::Pressed
                            }
                            VirtualKeyCode::D => {
                                movement.right = input.state == ElementState::Pressed
                            }
                            VirtualKeyCode::S => {
                                movement.backward = input.state == ElementState::Pressed
                            }
                            VirtualKeyCode::W => {
                                movement.forward = input.state == ElementState::Pressed
                            }
                            VirtualKeyCode::LShift => {
                                movement.downward = input.state == ElementState::Pressed
                            }
                            VirtualKeyCode::Space => {
                                movement.upward = input.state == ElementState::Pressed
                            }
                            _ => (),
                        }
                    }
